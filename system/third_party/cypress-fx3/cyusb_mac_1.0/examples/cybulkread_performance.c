...
int main(int argc, char **argv)
{
    ...
    if (argc != 2){
        printf ("Usage ./cybulkread_performance <Count of 4mb records sent to device> \n Ex: ./cybulkread_performance 800 \n Host will send 4 * 800 mb to Device \n"); 
        return -1;
    }
    ...
    while (count < countLimit){
        rStatus = cyusb_bulk_transfer(h1, ep_in, buf, nbr, &transferred, 1000);
            if ((rStatus != 0)|| (transferred != nbr))
                printf ("Error in Read..Read only %d bytes out of %d bytes...\n", transferred, nbr);
    ...
    if ( rStatus == 0 ) {
        ...
        printf ("Read Successfully (%d * %d) bytes in %ld.%ld seconds \n",nbr,count,sec,usec);
    }
    else {
