...
AsyncScanner::AsyncScanner(
    ...
    : Scanner(systemParameters, laser, delays, ustx, cameras, octopusManager),
      xStage_(xStage), yStage_(yStage), zStage_(zStage) {
}
...
bool AsyncScanner::initializeStage(Stage* stage, int comPort) {
  if (!stage) return true;
  if (!stage->init(comPort)) return false;
  stage->resetController();
...
bool AsyncScanner::scan() {
  ...
  double frameGatePeriod_ms = double(numAxialSteps * laserClockPeriod_ms);
